...
import { FITNESS_CONFIG, type FitnessCategoryId } from "@/lib/assessment/fitness";
import {
  overallScore,
  categoryScores,
  getMaturityLevel,
...
const C = FITNESS_CONFIG;

// Icons live in the UI layer, mirroring how Football keeps SECTION_ICONS out of
// its pure data module.
const CATEGORY_ICONS: Record<FitnessCategoryId, ComponentType<{ className?: string }>> = {
  attract: Magnet,
  activate: Zap,
  engage: Activity,
  retain: Anchor,
...
export function FitnessAssessment() {
  ...
  function onFormSuccess(email: string) {
    ...
    setStep("results");
    trackAuditCompleted("fitness", level.id);
    trackLeadSubmitted("self-assessment:fitness");
    trackAuditResultViewed("fitness", level.id);
    scrollTop();
  }
    ...
    <>
      ...
      {step === "form" && (
        <LeadForm
          ...
          sport="fitness"
          reportDescription="Wynik i pełny raport PDF (Fitness Growth Score, wyniki 6 obszarów i największe szanse) wyślemy na podany adres email. Po wysłaniu zobaczysz też wynik tutaj, na stronie."
          chipScore={`${score} / 100`}
      ...
      <div className="container relative py-16 md:py-20">
        <div className="mx-auto max-w-3xl">
          <Badge variant="soft" className="mb-5 gap-1.5 px-3 py-1">
            ...
            Self-Audit dla klubów fitness i siłowni
          </Badge>
          ...
          <p className="mt-5 max-w-2xl text-[17px] leading-relaxed text-muted-foreground">
            ...
            wygląda ścieżka członka w Waszym klubie - od pierwszego kontaktu,
          ...
          <div className="mx-auto max-w-5xl">
                ...
                <div className="mt-4 space-y-4">
                  {cats.map((c) => (
                    <CategoryBar
                      key={c.id}
                      label={c.label}
                      icon={CATEGORY_ICONS[c.id as FitnessCategoryId]}
                      score={c.score}
                    />
                  ))}
                </div>
            ...
            <div className="mt-5 rounded-2xl border border-navy-100 bg-white p-6 shadow-[0_8px_24px_-16px_rgba(15,23,42,0.12)] sm:p-8">
              ...
              <ul className="mt-4 space-y-3">
                {top3.map((c, i) => {
                  const Icon = CATEGORY_ICONS[c.id as FitnessCategoryId];
                  return (
                    <li
                      key={c.id}
          ...
          <div className="mx-auto max-w-3xl text-center">
            <h2 className="text-balance text-[32px] font-semibold leading-tight tracking-tight text-white sm:text-[40px]">
              ...
              <span className="text-navy-300">
                Czas sprawdzić, co o klubie myślą Wasi członkowie.
              </span>
            </h2>
            <p className="mx-auto mt-5 max-w-2xl text-[16px] leading-relaxed text-navy-200">
              ...
              satysfakcji weryfikuje te hipotezy u samych członków - i pokazuje,
