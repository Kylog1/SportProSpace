// Email templates for the Fitness Self-Audit lead magnet.
// Mirrors lib/assessment/emails.ts (Football) in tone and structure.
...
export type FitnessUserEmailInput = {
  name: string;
  organization: string;
  answers: Record<string, number>;
};

export function buildFitnessUserEmail(input: FitnessUserEmailInput): {
  subject: string;
  html: string;
  text: string;
} {
...
export type FitnessAdminEmailInput = {
  name: string;
  email: string;
  organization: string;
  phone?: string;
...
export function buildFitnessAdminEmail(input: FitnessAdminEmailInput): {
  subject: string;
  html: string;
  text: string;
} {
  ...
  const subject = `Nowy lead (Self-Audit Fitness) - ${input.organization} - ${level.name} (${score}/100)`;
...
<div style="font-family:-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#0f172a;">
  <h2 style="font-size:18px;font-weight:600;margin:0 0 6px;">Nowy lead - Self-Audit Fitness</h2>
  <p style="color:#64748b;font-size:13px;margin:0 0 18px;">sportspacepro.pl/self-assessment/fitness</p>
  ...
  <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;margin-bottom:18px;">
    <tbody>
      ...
      ${phoneRow}
      <tr style="background:#f8fafc;"><td style="padding:5px 12px;color:#64748b;font-size:13px;">Dyscyplina</td><td style="padding:5px 12px;font-size:13px;">fitness</td></tr>
    </tbody>
  </table>
  ...
  const text = [
    `Nowy lead - Self-Audit Fitness (sportspacepro.pl)`,
    ``,
