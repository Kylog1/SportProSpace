// Server-side PDF generator for the Fitness Self-Audit.
//
// Built on lib/assessment/pdf-shared.ts. The Football generator (pdf.ts) keeps
// its own private copies of these primitives on purpose: it is a protected,
// shipped artefact and refactoring it buys nothing a new discipline needs.
// If Football is ever revisited, its helpers can be collapsed into pdf-shared.
...
export type GenerateFitnessPdfInput = {
  name: string;
  email: string;
  organization: string;
...
export function generateFitnessPDF(
  input: GenerateFitnessPdfInput
): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      const doc = new PDFDocument({
        ...
        bufferPages: true,
        info: {
          Title: "Self-Audit Fitness - Sport Space Pro",
      ...
      doc.font(FONT_REG).fontSize(9.5).fillColor(COLORS.muted)
        .text(
          "Self-Audit pokazuje, gdzie są luki - na podstawie tego, co widzi zarząd. Pełna diagnoza przyczyn, dane od samych członków i plan działań to zakres Growth Audit. Umów rozmowę: sportspacepro.pl",
          ML + 20, y + 32,
